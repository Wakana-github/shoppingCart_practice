...
function ShoppingCart() {
  ...
  return (
    <>
    {cartItems.length !== 0 && (
    <div className={`w-[300px] h-screen bg-gray-200 fixed top-0 z-30 
        border-l-4 border-red-200 rounded-tl-lg 
        ...
        <div className='w-full h-16 bg-white absolute left-0 top-0 z-10 grid place-items-center border rounded-lg'>
            ...
            <button className='w-9 h-9 bg-yellow-400 absolute right-3 z-20 grid place-items-center 
                border-2 rounded-full hover:bg-yellow-500 transition-colors'
                onClick={()=>setIsOpen(false)}>
                <XIcon className='text-white'/>

            </button>
        </div>
            <button className='w-9 h-9 bg-yellow-400 absolute -left-14 top-3 z-20 grid 
                place-items-center border-2 rounded-full' onClick={()=>setIsOpen(true)}>
