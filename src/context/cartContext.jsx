...
export const CartProvider = ({children}) => {
    ...
    const addToCart = (item) =>{
        setAllItems((prevItems) => {
            return prevItems.map((prevItems) => {
                ...
                return prevItems.id === item.id? {...prevItems, inCart:true} : prevItems
            })
        })
    }
    ...
    const removeFromCart = (item) =>{
        setAllItems((prevItems)=>{
            return prevItems.map((prevItems) => {
                return prevItems.id === item.id ?{...prevItems, inCart: false, quantity: 1} :prevItems
            })
        })
    }
    ...
    return(
        <CartContext.Provider 
            value={{allItems, setItems, addToCart, removeFromCart, updateQuantity,
                setLocalStorage, setCartItemsFromStorage }}>
            {children}
        </CartContext.Provider>
    )
}
